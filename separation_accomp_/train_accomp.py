...
class TrainConfig:
    ...
    alpha_similarity: float = 5.0 # 旋律/伴奏互斥相似度權重 (處罰伴奏中的旋律殘留)
    melody_weight: float = 1.0    # 被扣除部分的參考權重
    accomp_weight: float = 7.5    # 伴奏擬合權重
...
def main():
    # 初始化 ClearML Task
    task = Task.init(project_name='piano-mir-melody-separation', task_name='Accomp-Separation-Training')
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        plt.legend(); plt.grid(True); plt.savefig('loss_curve_acc.png'); plt.close()
