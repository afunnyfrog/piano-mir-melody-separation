...
from utils_accomp.loss import AudioSeparationLoss
...
def main():
    ...
    with mlflow.start_run(run_name='Accomp-Separation-Training') as run:
        mlflow.log_params(asdict(cfg))
        mlflow.set_tag("Task Type", "Accompaniment Separation")
            ...
            for epoch in range(start_epoch, start_epoch + cfg.run_epochs):
                ...
                for batch_idx, (waveforms, targets, target_audios) in enumerate(train_loader):
                    ...
                    if predictions.shape[3] != targets.shape[3]:
                        ...
                        predictions, targets, pred_mels = predictions[:, :, :, :min_time], targets[:, :, :, :min_time], pred_mels[:, :, :, :min_time]
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    # 記錄最佳模型到 MLflow
                    mlflow.pytorch.log_model(model, name="best_model", registered_model_name="Piano-Accomp-Separation", export_model=True)
